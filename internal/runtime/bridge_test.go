...
func TestMain(m *testing.M) {
	if handled, code := nativeTestPrivate(); handled {
		os.Exit(code)
	}
	if len(os.Args) == 2 && (os.Args[1] == "--runtime-parent-fixture" || os.Args[1] == "--runtime-parent-blocked") {
		...
		if os.Args[1] == "--runtime-parent-blocked" {
...
func nativeParentRequest(t *testing.T, operation uint64, terminal bool) api.SessionStartRequest {
	t.Helper()
	...
	rootID, projectID := must(nativeTestDirectory(rootFile)), must(nativeTestDirectory(projectFile))
	...
	inv.Execution = must(api.NewArgvExecution(api.ArgvExecutionData{Executable: "./user-fixture" + nativeTestSuffix(), Arguments: []string{"--runtime-parent-fixture"}}))
	inv.Environment = must(api.NewEnvironmentPolicy(api.EnvironmentPolicyData{InheritBase: true, Set: []api.EnvironmentEntry{
...
func nativeParentEngine(t *testing.T) *sessions {
	t.Helper()
	r := newSessions(nativeTestStarter(), os.Environ(), sessionBudgets{grace: 30 * time.Millisecond, force: nativeTestForce(), shutdown: 8 * time.Second})
	t.Cleanup(func() {
...
func TestNativeParentPipesAndPersistentStart(t *testing.T) {
	request := nativeParentRequest(t, 11, false)
...
func TestNativeParentTerminalControlAndRestart(t *testing.T) {
	request := nativeParentRequest(t, 21, true)
	r := nativeParentEngine(t)
	...
	must(r.Write(context.Background(), must(api.NewSessionWriteRequest(api.SessionWriteRequestData{SessionID: id, Bytes: []byte("size" + nativeTestEnter())}))))
...
func TestNativeParentFailedStartPreservesCodeAndIdentity(t *testing.T) {
	request := nativeParentRequest(t, 31, false).Data()
	r := nativeParentEngine(t)
	inv := request.Invocation.Data()
