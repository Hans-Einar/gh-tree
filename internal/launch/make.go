...
func (MakeProvider) Discover(root string)([]Candidate,error){
	...
	set:=map[string]bool{};scanner:=bufio.NewScanner(file);for scanner.Scan(){line:=strings.TrimSpace(scanner.Text());if line==""||strings.HasPrefix(line,"#")||strings.HasPrefix(line,"\t")||strings.HasPrefix(line,"."){continue};idx:=strings.Index(line,":");if idx<=0{continue};left:=strings.TrimSpace(line[:idx]);if strings.ContainsAny(left,"=$%")||strings.Contains(left,"/"){continue};for _,name:=range strings.Fields(left){if validMakeTarget(name){set[name]=true}}};if err:=scanner.Err();err!=nil{return nil,err}
