...
func TestStatusAndCommitHistory(t *testing.T) {
	...
	m := NewManager(process.ExecRunner{}, fixture.mainRoot)
	status, err := m.Status(context.Background(), fixture.targetRoot)
	...
	sha, err := m.Commit(context.Background(), fixture.targetRoot, "v2 commit")
	if err != nil { t.Fatal(err) }
	if len(sha) != 40 { t.Fatalf("sha=%q", sha) }
	commits, err := m.Commits(context.Background(), fixture.targetRoot, "HEAD", 10, 0)
	if err != nil { t.Fatal(err) }
...
func TestCreateAndRetargetWorktree(t *testing.T) {
	fixture := setupRepository(t, false)
	m := NewManager(process.ExecRunner{}, fixture.mainRoot)
	path := filepath.Join(filepath.Dir(fixture.mainRoot), "created")
	info, err := m.Create(context.Background(), CreateRequest{Path: path, StartPoint: fixture.initialSHA, Branch: "local/created"})
	if err != nil { t.Fatal(err) }
	...
	if _, err := m.Checkout(context.Background(), CheckoutRequest{Path: path, Revision: fixture.prSHA, Detach: true}); err == nil || !strings.Contains(err.Error(), "dirty") {
		t.Fatalf("expected dirty refusal, got %v", err)
	}
	if err := os.Remove(filepath.Join(path, "dirty.txt")); err != nil { t.Fatal(err) }
	info, err = m.Checkout(context.Background(), CheckoutRequest{Path: path, Revision: fixture.prSHA, Detach: true})
	if err != nil { t.Fatal(err) }
...
func TestPreparePullRequestAndCreatePRWorktree(t *testing.T) {
	fixture := setupRepository(t, false)
	m := NewManager(process.ExecRunner{}, fixture.mainRoot)
	ref, err := m.PreparePullRequest(context.Background(), 60, fixture.prSHA)
	if err != nil { t.Fatal(err) }
	if ref != "refs/gh-tree/pr-60" { t.Fatalf("ref=%q", ref) }
	path := filepath.Join(filepath.Dir(fixture.mainRoot), "pr-worktree")
	info, err := m.Create(context.Background(), CreateRequest{Path: path, StartPoint: ref, Branch: "local/pr-60"})
	if err != nil { t.Fatal(err) }
