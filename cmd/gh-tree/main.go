...
func run(ctx context.Context,args []string)error{
	...
	service:=&app.Service{GitHub:client,Worktrees:manager,GraphReader:graphReader,DiffReader:diffReader,LocalRoot:localSelectedRoot,Launcher:launch.NewSessionManager(1000),Terminal:terminal.NewManager(256*1024),LaunchRegistry:launch.DefaultRegistry()};defer func(){_ = service.StopAllConsoles()}()
	...
	program:=tea.NewProgram(tui.WithV313UX(model),tea.WithAltScreen());if _,err:=program.Run();err!=nil{return fmt.Errorf("run terminal UI: %w",err)};return nil
}
