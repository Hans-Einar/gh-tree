...
func run(ctx context.Context,args []string)error{
	...
	service:=&app.Service{GitHub:client,Worktrees:manager,GraphReader:graphReader,DiffReader:diffReader,LocalRoot:localSelectedRoot,Launcher:launch.NewSessionManager(1000),LaunchRegistry:launch.DefaultRegistry()};defer func(){_ = service.StopAllConsoles()}()
	...
	program:=tea.NewProgram(tui.WithV312UX(model),tea.WithAltScreen());if _,err:=program.Run();err!=nil{return fmt.Errorf("run terminal UI: %w",err)};return nil
}
