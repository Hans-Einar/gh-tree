...
func run(ctx context.Context,args []string)error{
	...
	var manager *worktree.Manager;var graphReader *graph.Reader
	if rootErr==nil{localClient:=ghapi.NewClient(runner,localRoot);localRepo,localErr:=localClient.ResolveRepository(ctx,"");if localErr==nil&&strings.EqualFold(localRepo,repo){manager=worktree.NewManager(runner,localRoot);graphReader=graph.NewReader(runner,localRoot)}}
	service:=&app.Service{GitHub:client,Worktrees:manager,GraphReader:graphReader}
	if *graphFlag {
		if graphReader==nil{return errors.New("--graph requires running inside the selected local repository")}
		program:=tea.NewProgram(graphui.New(repo,service),tea.WithAltScreen());if _,err:=program.Run();err!=nil{return fmt.Errorf("run Git graph UI: %w",err)};return nil
	}
